  ...
  return (
    <>
      ...
      {activeCardFace === 'glitch' && (
        <div 
          className="absolute inset-0 z-0 bg-black overflow-hidden pointer-events-none animate-[glitch-pop-anim_0.8s_infinite]"
        ...
        >
          ...
          <div className="absolute top-1/2 left-0 right-0 h-8 bg-cyan-400/20 mix-blend-color-dodge blur-[2px] animate-[glitch-blur_1.2s_infinite]" style={{ willChange: 'transform' }}></div>
          <div className="absolute top-1/4 left-0 right-0 h-4 bg-fuchsia-400/10 mix-blend-screen -rotate-3 animate-[glitch-blur_20s_infinite_reverse]" style={{ willChange: 'transform' }}></div>
          <div className="absolute inset-0 border border-fuchsia-400/20 mix-blend-screen animate-[glitch-flicker_0.15s_infinite]" style={{ willChange: 'opacity' }}></div>
        </div>
      )}
      ...
      {activeCardFace === 'cosmic_void' && (
        ...
        >
          <div className="absolute w-20 h-20 bg-indigo-500/15 rounded-full blur-xl -top-6 -left-6" style={{ willChange: 'transform' }}></div>
          <div className="absolute w-20 h-20 bg-purple-500/15 rounded-full blur-xl -bottom-6 -right-6 animate-pulse" style={{ willChange: 'transform, opacity' }}></div>
        </div>
      )}

      {activeCardFace === 'crimson_fire' && (
        <div 
          className="absolute inset-0 z-0 overflow-hidden pointer-events-none bg-gradient-to-b from-[#1a0505] to-[#040000]"
          style={{ willChange: 'transform', transform: 'translate3d(0, 0, 0)', backfaceVisibility: 'hidden' }}
        >
          {CRIMSON_FIRE_SPECS.map((fire, i) => (
            <div 
              key={i} 
            ...
            >
              <div 
                className={`rounded-full bg-gradient-to-t ${fire.bg} ${fire.size} blur-[3px] animate-[fire-sway-anim_3s_infinite_ease-in-out]`}
      ...
      {activeCardFace === 'royal_gold' && (
        <div 
          className="absolute inset-0 z-0 overflow-hidden pointer-events-none rounded-sm mix-blend-overlay"
          style={{ willChange: 'transform', transform: 'translate3d(0, 0, 0)', backfaceVisibility: 'hidden' }}
        >
          {/* Shimmer overlay */}
          <div 
      ...
      {activeCardFace === 'beach_breeze' && (
        <div 
          className="absolute inset-0 z-0 overflow-hidden pointer-events-none rounded-sm"
          style={{ willChange: 'transform', transform: 'translate3d(0, 0, 0)', backfaceVisibility: 'hidden' }}
        >
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 h-4 bg-white/70 blur-[0.5px] animate-pulse" style={{ borderRadius: '0 0 50% 50% / 0 0 35% 35%', willChange: 'transform, opacity' }}></div>
          ...
          <div className="absolute inset-0 z-[1] overflow-hidden pointer-events-none">
            {WATER_SPARKLES.map((sparkle, idx) => (
              <div 
