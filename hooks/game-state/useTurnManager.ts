...
export const useTurnManager = ({
  ...
  const resolveTiebreaker = (state: GameState): GameState => {
    ...
    if (p1.life > p2.life) {
      ...
      const getFieldValue = (p: PlayerState) =>
        p.field.reduce(
          (acc, f) => acc + (f.card.rank === "A" ? 1 : f.card.numericValue),
          0,
        );
  ...
  const drawCards = async (playerId: number, count: number) => {
    ...
    const isSpectate =
      startState.players[0].isCpu && startState.players[1].isCpu;
    const isHotseat =
      !startState.players[1].isCpu || startState.players[0].isCpu;

    // In Online mode, the local player is always "bottom". In Hotseat/Solo, it depends on whose turn it is.
    const viewId = isOnline
      ? localPlayerId
      : isHotseat && !isSpectate
        ? startState.turnPlayer
        : 0;
    ...
    const shouldShowFace = isOnline
      ? playerId === localPlayerId
      : !player.isCpu ||
        isSpectate ||
        (isHotseat && playerId === startState.turnPlayer);

    // Extract cards from deck model
    const deck =
      startState.mode === "STREET" || startState.mode === "TUTORIAL"
        ? startState.deck
        : player.library;
    let cardsToDraw = deck.slice(0, Math.min(count, deck.length));

    if (startState.campaignChallenge === 'WEAK_SOLDIERS' && playerId === 0) {
      cardsToDraw = cardsToDraw.map(c => {
        const charToRankMap: Record<string, Rank> = {
          '6': Rank.Ace,
          '7': Rank.Two,
          '8': Rank.Three,
          '9': Rank.Four,
    ...
    if (isOpeningDraw) {
      ...
      const promises = cardsToDraw.map((card, index) => {
        return new Promise<void>((resolve) => {
          setTimeout(() => {
            let startRect = isBottom
              ? refs.bottomDeckRef.current?.getBoundingClientRect()
              : refs.topDeckRef.current?.getBoundingClientRect();
            let targetRect = isBottom
      ...
      setGameState((prev) =>
        prev
          ? {
              ...
              logs: addLog(
                prev,
                `${player.name} drew ${cardsToDraw.length} cards.`,
              ),
            }
          : null,
      );
    ...
    if (failuresNeeded > 0) {
          ...
          pl.id === playerId
            ? {
                ...
                consecutiveDrawFailures:
                  pl.consecutiveDrawFailures + failuresNeeded,
              }
            : pl,
        );
        const nextState = {
          ...prev,
          players: nextPlayers,
          logs: addLog(prev, `${player.name} deck empty!`),
        };
        if (
          nextPlayers[0].consecutiveDrawFailures > 0 &&
          nextPlayers[1].consecutiveDrawFailures > 0
        ) {
      ...
      setTimeout(() => {
        if (gameStateRef.current) checkGameOver(gameStateRef.current);
      }, 100);
    }
  };
    ...
    setGameState((prev) => {
      ...
      switch (targetPhase) {
        case Phase.UPKEEP:
          newState.players.forEach((p) => {
            if (p.id === newState.turnPlayer) {
              p.field.forEach((c) => {
                c.isTapped = false;
                if (!(newState.turnCount === 1 && newState.campaignChallenge === 'AMBUSH')) {
          ...
          newState.pendingAttackers = [];
          newState.pendingBlocks = {};
          newState.selectedCardId = null;
          newState.recentDamage = {};
          newState.activeCombatCardId = null;
          player.hasAttackedThisTurn = false;
          newState.logs = addLog(
            newState,
            `Turn ${newState.turnCount}: ${player.name}'s Upkeep.`,
          );
          ...
          setTimeout(() => {
            drawCards(player.id, 1).then(() => {
              setGameState((current) => {
                ...
                const isTurn1FirstPlayer =
                  current.turnCount === 1 &&
                  current.turnPlayer === current.startingPlayerId;
                ...
                if (isTurn1FirstPlayer || hasMaxResources) {
                  ...
                  return {
                    ...current,
                    phase: Phase.RESOURCE_START,
                    logs: addLog(current, "Resource Step."),
                  };
                }
              });
            });
        ...
        case Phase.RESOURCE_START:
          break;
        case Phase.MAIN:
          newState.logs = addLog(newState, "Main Phase.");
          break;
        case Phase.ATTACK_DECLARE:
          newState.logs = addLog(newState, "Declare Attackers.");
          break;
  ...
  const performEndTurn = () => {
    setGameState((prev) => {
      ...
      if (
        prev.mode === "TUTORIAL" &&
        prev.tutorialState?.lessonId === "lesson-2"
      ) {
        if (prev.turnPlayer === 0) {
          ...
          const nextPlayers = prev.players.map((p) => ({
            ...
            resources:
              p.id === 0
                ? p.resources.map((r) => ({ ...r, isTapped: false }))
                : p.resources,
          }));
          const nextState = {
            ...prev,
      ...
      const nextTurnCount =
        nextPlayerIdx === prev.startingPlayerId
          ? prev.turnCount + 1
          : prev.turnCount;
      const nextState = {
        ...prev,
        turnPlayer: nextPlayerIdx,
