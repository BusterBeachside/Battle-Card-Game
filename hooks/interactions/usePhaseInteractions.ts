...
interface UsePhaseInteractionsProps {
...
export const usePhaseInteractions = ({
  gameState,
  actions,
  effects,
  tutorial,
  ...
  const handleCardClick = async (
    ...
    if (
      !fromCpu &&
      currentState.players[activePlayerId].isCpu &&
      ownerId === activePlayerId
    ) {
      if (currentState.phase !== Phase.INIT_SELECT) return;
    }
    ...
    if (currentState.phase === Phase.INIT_SELECT && location === "HAND") {
      if (ownerId === activePlayerId) {
        actions.setGameState((prev: any) => {
          ...
          let newIds = isSelected
            ? prev.initSelectedIds.filter((id: string) => id !== card.id)
            : [...prev.initSelectedIds, card.id];
          if (newIds.length > 3) newIds.pop();
          return { ...prev, initSelectedIds: newIds };
        });
        if (currentState.mode === "TUTORIAL")
          tutorial.advanceTutorialStep("CLICK_CARD", card.id);
      }
    } else if (
      currentState.phase === Phase.RESOURCE_ADD_SELECT &&
      location === "HAND" &&
      ownerId === activePlayerId
    ) {
      const targetResourceContainer = document.getElementById(
        `resource-container-${ownerId}`,
      );
      let cardEl = document.getElementById(card.id);

      playSound("play_resource");

      if (cardEl && targetResourceContainer) {
        const activePlayer = currentState.players[activePlayerId];
        await effects.triggerFlyer(
          card,
          cardEl.getBoundingClientRect(),
          targetResourceContainer.getBoundingClientRect(),
          false,
          undefined,
          undefined,
          activePlayer?.cardBack,
          activePlayer?.cardFace,
        );
      }
      ...
      actions.setGameState((prev: any) => {
        ...
        return {
          ...
          logs: addLog(
            prev,
            `Converted [${resCard.rank}${resCard.suit}] into a resource.`,
          ),
        };
      });
      ...
      currentState.phase === Phase.RESOURCE_SWAP_SELECT_HAND &&
      location === "HAND"
    ) {
      actions.setGameState((prev: any) => ({
        ...prev!,
        selectedCardId: card.id,
        phase: Phase.RESOURCE_SWAP_SELECT_PILE,
        logs: addLog(prev!, "Select a resource to swap with."),
      }));
      if (currentState.mode === "TUTORIAL")
        tutorial.advanceTutorialStep("CLICK_CARD", card.id);
    } else if (
      currentState.phase === Phase.RESOURCE_SWAP_SELECT_PILE &&
      location === "RESOURCE"
    ) {
      if (currentState.selectedCardId) {
        // Determine layout logic to find correct hand ref (Bottom vs Top player)
        const isHotseat =
          !currentState.players[1].isCpu || currentState.mode === "SANDBOX";
        ...
        const handContainer = isBottom
          ...
          : refs.cpuHandRef.current;
        const handCard = currentState.players[activePlayerId].hand.find(
          (c: any) => c.id === currentState.selectedCardId,
        );

        if (handCard && resCardEl && handContainer) {
          let startRect = handCardEl
            ? handCardEl.getBoundingClientRect()
            : null;
          if (!startRect) {
            // Fallback: Use center of hand container if specific card element not found (e.g. CPU hand scaling issues)
          ...
          const activePlayer = currentState.players[activePlayerId];
          const p1 = effects.triggerFlyer(
            handCard,
            startRect,
            resCardEl.getBoundingClientRect(),
            false,
            undefined,
            undefined,
            activePlayer?.cardBack,
          ...
          const p2 = effects.triggerFlyer(
            card,
            resCardEl.getBoundingClientRect(),
            handContainer.getBoundingClientRect(),
            false,
            undefined,
            undefined,
            activePlayer?.cardBack,
      ...
      actions.setGameState((prev: any) => {
        ...
        const handIdx = p.hand.findIndex(
          (c: any) => c.id === prev.selectedCardId,
        );
        ...
        if (handIdx > -1 && resIdx > -1) {
          ...
          const resFieldCard = p.resources[resIdx];
      ...
      currentState.phase === Phase.ATTACK_DECLARE &&
      location === "FIELD" &&
      ownerId === activePlayerId
    ) {
      actions.setGameState((prev: any) => {
        if (!prev) return null;
        const p = prev.players[activePlayerId];
        const fieldCard = p.field.find(
          (fc: any) => fc.instanceId === instanceId,
        );
        if (!fieldCard || fieldCard.isTapped || fieldCard.isSummoningSick)
          return prev;
        const isAttacking = prev.pendingAttackers.includes(instanceId!);
        const newPending = isAttacking
          ? prev.pendingAttackers.filter((id: string) => id !== instanceId)
          : [...prev.pendingAttackers, instanceId!];
        return { ...prev, pendingAttackers: newPending as string[] };
      });
      if (currentState.mode === "TUTORIAL")
        tutorial.advanceTutorialStep("CLICK_CARD", card.id);
    } else if (
      currentState.phase === Phase.BLOCK_DECLARE &&
      location === "FIELD" &&
      ownerId === activePlayerId
    ) {
      ...
      currentState.phase === Phase.MAIN &&
      location === "HAND" &&
      ownerId === activePlayerId
    ) {
      const player = currentState.players[activePlayerId];
      const untappedRes = player.resources.filter(
        (r: any) => !r.isTapped,
      ).length;
      ...
      if (untappedRes < card.cost) {
        actions.setGameState((prev: any) => ({
          ...prev!,
          logs: addLog(prev!, "Not enough resources!"),
        }));
        return;
      }
      ...
      if (["K", "Q"].includes(card.rank)) {
        actions.setGameState((prev: any) => ({
          ...prev!,
          logs: addLog(prev!, "Drag Face Cards to target!"),
        }));
      } else {
        actions.playCard(card);
        if (currentState.mode === "TUTORIAL")
          tutorial.advanceTutorialStep("PLAY_CARD", card.id);
      }
    }
  };
    ...
    if (action === "ATTACK_PHASE") {
      ...
      if (gameState?.mode === "TUTORIAL") {
        ...
        tutorial.advanceTutorialStep(
          "CLICK_UI_BUTTON",
          "btn-confirm-attackers",
        );
      }
      actions.confirmAttack();
    } else if (action === "CONFIRM_BLOCK") {
      if (gameState?.mode === "TUTORIAL") {
        // STRICT VALIDATION: If current step requires DECLARE_BLOCK, do NOT allow confirm
        // This fixes mobile touch bypassing normal checks
        const currentStep =
          gameState.tutorialState?.steps[
            gameState.tutorialState.currentStepIndex
          ];
        if (currentStep && currentStep.requiredAction === "DECLARE_BLOCK") {
          console.warn(
            "Tutorial Blocking: Cannot confirm blocks before declaring required block.",
          );
          return;
        }

        if (
          !tutorial.isInteractionAllowed("btn-confirm-blocks") &&
          !tutorial.isInteractionAllowed("btn-tutorial-next")
        )
          return;
  ...
  return { handleCardClick, handleConfirmInitSelection, handlePhaseAction };
};
